...
if __name__ == '__main__':
    ...
    start = (date.today() - timedelta(15)).strftime("%Y%m%d")
    cur = conn_43.cursor()
    sql = """
    ...
    for dt in date_range:
        ...
        AAA9Minfo = AAA9M(dt.strftime("%Y%m%d"))
        if AAA9Minfo is not None:
            calList.append([dt.strftime("%Y%m%d"), qualificationCal(df), emotionCal(df), float(AAA9Minfo[1])])
    ...
    if calList.__len__() > 0:
        ...
        df[['updatetime', 'qualification']].to_sql("QualificationIndex", engine_InvestSystem, index=False,
                                                   if_exists='append')
